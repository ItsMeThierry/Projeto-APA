    ...
    for(int i = 0; i < num_pistas; i++){
        ...
        for (int origem = 0; origem < size - 1; origem++) {
            for (int destino = origem + 1; destino < size; destino++){
                ...
                if(origem > 0) t += matrix[sol.pistas[i][origem-1].id - 1][sol.pistas[i][destino].id - 1];
                if(t < sol.pistas[i][destino].t_decolagem){ t += sol.pistas[i][destino].t_decolagem - t; }
                else if(t > sol.pistas[i][destino].t_decolagem){ multa += sol.pistas[i][destino].multa * (t - sol.pistas[i][destino].t_decolagem); }
                t += sol.pistas[i][destino].duracao;

                for(int v = origem; v < destino; v++){
                    if(v > 0) t += (v - 1 == origem) ? matrix[sol.pistas[i][destino].id - 1][sol.pistas[i][v].id - 1] : matrix[sol.pistas[i][v-1].id - 1][sol.pistas[i][v].id - 1];
                    if(t < sol.pistas[i][v].t_decolagem){ t += sol.pistas[i][v].t_decolagem - t; }
                    else if(t > sol.pistas[i][v].t_decolagem){ multa += sol.pistas[i][v].multa * (t - sol.pistas[i][v].t_decolagem); }
                    t += sol.pistas[i][v].duracao;
                }

                t += (destino - 1 == origem) ? matrix[sol.pistas[i][destino].id - 1][sol.pistas[i][origem].id - 1] : matrix[sol.pistas[i][destino-1].id - 1][sol.pistas[i][origem].id - 1];
                if(t < sol.pistas[i][origem].t_decolagem){ t += sol.pistas[i][origem].t_decolagem - t; }
                else if(t > sol.pistas[i][origem].t_decolagem){ multa += sol.pistas[i][origem].multa * (t - sol.pistas[i][origem].t_decolagem); }
                t += sol.pistas[i][origem].duracao;

                for(int v = destino; v < size; v++){
                    if(v > 0) t += (v - 1 == destino) ? matrix[sol.pistas[i][origem].id - 1][sol.pistas[i][v].id - 1] : matrix[sol.pistas[i][v-1].id - 1][sol.pistas[i][v].id - 1];
                    if(t < sol.pistas[i][v].t_decolagem){ t += sol.pistas[i][v].t_decolagem - t; }
                    else if(t > sol.pistas[i][v].t_decolagem){ multa += sol.pistas[i][v].multa * (t - sol.pistas[i][v].t_decolagem); }
                    t += sol.pistas[i][v].duracao;
                }
                
                if (multa < menor_multa && multa < multa_antes) {
                    menor_multa = multa;
                    pista = i;
                    pos_origem = origem;
                    pos_destino = destino;
...
void re_insertion(solucao &sol, int** matrix, int num_pistas) {
    ...
    // int pos_origem = -1;
    // int pos_destino = -1;
    std::vector<voo> pista_modificada;
    ...
    for(int i = 0; i < num_pistas; i++) {
        ...
        int multa_antes = calcula_multa_pista(sol.pistas[i], matrix);
        std::vector<voo> pista_simulacao = sol.pistas[i];
        ...
        for(int origem = 0; origem < size; origem++){
            for(int destino = 0; destino < size; destino++){
                ...
                pista_simulacao.erase(pista_simulacao.begin() + origem);
                pista_simulacao.insert(pista_simulacao.begin() + destino, sol.pistas[i][origem]);

                int multa = calcula_multa_pista(pista_simulacao, matrix);

                if(multa < multa_antes && multa < menor_multa){
                    menor_multa = multa;
                    pista = i;
                    pista_modificada = pista_simulacao;
                }
    ...
    if(pista != -1){
        int multa_antes = calcula_multa_pista(sol.pistas[pista], matrix);
        sol.pistas[pista] = pista_modificada;
    ...
    while(k <= 6){
        switch(k){
            ...
            case 3:
                ...
                break;
            case 4:
                re_insertion_3(otimo, matrix, num_pistas);
                break;
            case 5:
            ...
            case 7:
                ...
                break;
            case 8:
                re_insertion_3_pistas(otimo, matrix, num_pistas);
                break;
        }
